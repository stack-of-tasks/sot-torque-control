...
JointTrajectoryGenerator::JointTrajectoryGenerator(const std::string& name)
  ...
  Entity::signalRegistration(m_qSOUT << m_dqSOUT << m_ddqSOUT << m_base6d_encodersSIN << m_fRightFootSOUT
                                     << m_fLeftFootSOUT << m_fRightHandSOUT << m_fLeftHandSOUT);

  /* Commands. */
  addCommand("init", makeCommandVoid2(*this, &JointTrajectoryGenerator::init,
                                      docCommandVoid2("Initialize the entity.", "Time period in seconds (double)",
                                                      "robotRef (string)")));

  addCommand("getJoint", makeCommandVoid1(
                             *this, &JointTrajectoryGenerator::getJoint,
                             docCommandVoid1("Get the current angle of the specified joint.", "Joint name (string)")));
  ...
  addCommand("startSinusoid",
             makeCommandVoid3(
                 *this, &JointTrajectoryGenerator::startSinusoid,
                 docCommandVoid3("Start an infinite sinusoid motion.", "(string) joint name",
                                 "(double) final angle in radians", "(double) time to reach the final angle in sec")));

  addCommand(
      "startTriangle",
      makeCommandVoid4(
          *this, &JointTrajectoryGenerator::startTriangle,
          docCommandVoid4("Start an infinite triangle wave.", "(string) joint name", "(double) final angle in radians",
                          "(double) time to reach the final angle in sec", "(double) time to accelerate in sec")));
  ...
  addCommand("startForceSinusoid",
             makeCommandVoid4(*this, &JointTrajectoryGenerator::startForceSinusoid,
                              docCommandVoid4("Start an infinite sinusoid force.", "(string) force name",
                                              "(int) force axis in [0, 5]", "(double) final 1d force in N or Nm",
  ...
  addCommand("startLinChirp",
             makeCommandVoid5(*this, &JointTrajectoryGenerator::startLinearChirp,
                              docCommandVoid5("Start a linear-chirp motion.", "(string) joint name",
                                              "(double) final angle in radians", "(double) initial frequency [Hz]",
                                              "(double) final frequency [Hz]", "(double) trajectory time [sec]")));

  addCommand(
      "startForceLinChirp",
      makeCommandVoid6(*this, &JointTrajectoryGenerator::startForceLinearChirp,
                       docCommandVoid6("Start a linear-chirp force traj.", "(string) force name", "(int) force axis",
                                       "(double) final force in N/Nm", "(double) initial frequency [Hz]",
                                       "(double) final frequency [Hz]", "(double) trajectory time [sec]")));
  ...
  addCommand(
      ...
      makeCommandVoid4(*this, &JointTrajectoryGenerator::moveForce,
                       docCommandVoid4("Move the force to the specified value with a minimum jerk trajectory.",
                                       "(string) force name", "(int) force axis", "(double) final force in N/Nm",
                                       "(double) time to reach the final force in sec")));

  addCommand("stop", makeCommandVoid1(
                         *this, &JointTrajectoryGenerator::stop,
                         docCommandVoid1(
                             "Stop the motion of the specified joint, or of all joints if no joint name is specified.",
                             "(string) joint name")));

  addCommand("stopForce", makeCommandVoid1(*this, &JointTrajectoryGenerator::stopForce,
                                           docCommandVoid1("Stop the specified force trajectort",
                                                           "(string) force name (rh,lh,lf,rf)")));
}

void JointTrajectoryGenerator::init(const double& dt, const std::string& robotRef) {
...
DEFINE_SIGNAL_OUT_FUNCTION(q, dynamicgraph::Vector) {
  ...
  getProfiler().start(PROFILE_POSITION_DESIRED_COMPUTATION);
  {
    ...
    if (m_status[0] == JTG_TEXT_FILE) {
      ...
      if (m_textFileTrajGen->isTrajectoryEnded()) SEND_MSG("Text file trajectory ended.", MSG_TYPE_INFO);
    } else {
      for (unsigned int i = 0; i < m_robot_util->m_nbJoints; i++) {
        ...
        if (m_currentTrajGen[i]->isTrajectoryEnded()) {
          ...
          m_noTrajGen[i]->set_initial_point(s(i));
          m_status[i] = JTG_STOP;
          SEND_MSG("Trajectory of joint " + m_robot_util->get_name_from_id(i) + " ended.", MSG_TYPE_INFO);
        }
      }
    }
  }
...
DEFINE_SIGNAL_OUT_FUNCTION(dq, dynamicgraph::Vector) {
  ...
  if (s.size() != static_cast<Eigen::VectorXd::Index>(m_robot_util->m_nbJoints)) s.resize(m_robot_util->m_nbJoints);
  if (m_status[0] == JTG_TEXT_FILE) {
    for (unsigned int i = 0; i < m_robot_util->m_nbJoints; i++) s(i) = m_textFileTrajGen->getVel()[i];
  } else
    for (unsigned int i = 0; i < m_robot_util->m_nbJoints; i++) s(i) = m_currentTrajGen[i]->getVel()(0);
...
DEFINE_SIGNAL_OUT_FUNCTION(ddq, dynamicgraph::Vector) {
  ...
  if (m_status[0] == JTG_TEXT_FILE) {
    for (unsigned int i = 0; i < m_robot_util->m_nbJoints; i++) s(i) = m_textFileTrajGen->getAcc()[i];
  } else
    for (unsigned int i = 0; i < m_robot_util->m_nbJoints; i++) s(i) = m_currentTrajGen[i]->getAcc()(0);
...
DEFINE_SIGNAL_OUT_FUNCTION(fRightFoot, dynamicgraph::Vector) {
  ...
  generateReferenceForceSignal("fRightFoot", static_cast<int>(m_robot_util->m_force_util.get_force_id_right_foot()), s,
                               iter);
  return s;
}

DEFINE_SIGNAL_OUT_FUNCTION(fLeftFoot, dynamicgraph::Vector) {
  generateReferenceForceSignal("fLeftFoot", static_cast<int>(m_robot_util->m_force_util.get_force_id_left_foot()), s,
                               iter);
  return s;
}

DEFINE_SIGNAL_OUT_FUNCTION(fRightHand, dynamicgraph::Vector) {
  generateReferenceForceSignal("fRightHand", static_cast<int>(m_robot_util->m_force_util.get_force_id_right_hand()), s,
                               iter);
  return s;
}

DEFINE_SIGNAL_OUT_FUNCTION(fLeftHand, dynamicgraph::Vector) {
  generateReferenceForceSignal("fLeftHand", static_cast<int>(m_robot_util->m_force_util.get_force_id_left_hand()), s,
                               iter);
  return s;
}

bool JointTrajectoryGenerator::generateReferenceForceSignal(const std::string& forceName, int fid,
  ...
  getProfiler().start(PROFILE_FORCE_DESIRED_COMPUTATION);
  {
    ...
    if (iter > m_iterForceSignals[fid]) {
      ...
      //            SEND_MSG("Compute force "+forceName+" with id "+toString(fid)+": "+toString(fr.transpose()),
      //            MSG_TYPE_DEBUG);
...
bool JointTrajectoryGenerator::isTrajectoryEnded() {
  ...
  if (m_status[0] == JTG_TEXT_FILE) {
    ...
    for (unsigned int i = 0; i < m_robot_util->m_nbJoints; i++) {
      if (!m_currentTrajGen[i]->isTrajectoryEnded()) {
        output = false;
        SEND_MSG("Trajectory of joint " + m_robot_util->get_name_from_id(i) + "not ended.", MSG_TYPE_INFO);
        return output;
      }
    }
  }
...
void JointTrajectoryGenerator::playTrajectoryFile(const std::string& fileName) {
  ...
  for (unsigned int i = 0; i < m_robot_util->m_nbJoints; i++)
    if (m_status[i] != JTG_STOP)
      return SEND_MSG("You cannot joint " + m_robot_util->get_name_from_id(i) + " because it is already controlled.",
                      MSG_TYPE_ERROR);
...
void JointTrajectoryGenerator::startTriangle(const std::string& jointName, const double& qFinal, const double& time,
                                             const double& Tacc) {
  if (!m_initSucceeded) return SEND_MSG("Cannot start triangle before initialization!", MSG_TYPE_ERROR);
  ...
  SEND_MSG("Set initial point of triangular trajectory to " + toString(m_triangleTrajGen[i]->getPos()),
           MSG_TYPE_DEBUG);
...
void JointTrajectoryGenerator::startForceSinusoid(const std::string& forceName, const int& axis, const double& fFinal,
                                                  const double& time) {
  if (!m_initSucceeded) return SEND_MSG("Cannot start force sinusoid before initialization!", MSG_TYPE_ERROR);

  unsigned int i;
  if (convertForceNameToForceId(forceName, i) == false) return;
  if (time <= 0.0) return SEND_MSG("Trajectory time must be a positive number", MSG_TYPE_ERROR);
  if (axis < 0 || axis > 5) return SEND_MSG("Axis must be between 0 and 5", MSG_TYPE_ERROR);
...
void JointTrajectoryGenerator::startLinearChirp(const string& jointName, const double& qFinal, const double& f0,
...
void JointTrajectoryGenerator::startForceLinearChirp(const string& forceName, const int& axis, const double& fFinal,
                                                     const double& f0, const double& f1, const double& time) {
  if (!m_initSucceeded) return SEND_MSG("Cannot start force linear chirp before initialization!", MSG_TYPE_ERROR);
...
void JointTrajectoryGenerator::moveJoint(const string& jointName, const double& qFinal, const double& time) {
  if (!m_initSucceeded) return SEND_MSG("Cannot move joint before initialization!", MSG_TYPE_ERROR);
...
void JointTrajectoryGenerator::moveForce(const string& forceName, const int& axis, const double& fFinal,
                                         const double& time) {
  if (!m_initSucceeded) return SEND_MSG("Cannot move force before initialization!", MSG_TYPE_ERROR);
...
void JointTrajectoryGenerator::stop(const std::string& jointName) {
  if (!m_initSucceeded) return SEND_MSG("Cannot stop joint before initialization!", MSG_TYPE_ERROR);
...
void JointTrajectoryGenerator::stopForce(const std::string& forceName) {
  if (!m_initSucceeded) return SEND_MSG("Cannot stop force before initialization!", MSG_TYPE_ERROR);
...
bool JointTrajectoryGenerator::convertJointNameToJointId(const std::string& name, unsigned int& id) {
  ...
  if (jid < 0) {
    ...
    std::stringstream ss;
    for (map<string, Index>::const_iterator it = m_robot_util->m_name_to_id.begin();
...
bool JointTrajectoryGenerator::convertForceNameToForceId(const std::string& name, unsigned int& id) {
  ...
  if (fid < 0) {
    ...
    std::stringstream ss;
    for (map<string, Index>::const_iterator it = m_robot_util->m_force_util.m_name_to_force_id.begin();
...
bool JointTrajectoryGenerator::isJointInRange(unsigned int id, double q) {
  ...
  if (q < jl.lower) {
    SEND_MSG("Joint " + m_robot_util->get_name_from_id(id) + ", desired angle " + toString(q) +
...
bool JointTrajectoryGenerator::isForceInRange(unsigned int id, const Eigen::VectorXd& f) {
  ...
  for (unsigned int i = 0; i < 6; i++)
    if (f[i] < fl.lower[i] || f[i] > fl.upper[i]) {
      SEND_MSG("Desired force " + toString(i) + " is out of range: " + toString(fl.lower[i]) + " < " + toString(f[i]) +
                   " < " + toString(fl.upper[i]),
               MSG_TYPE_ERROR);
      return false;
    }
