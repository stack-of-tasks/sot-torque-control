...
#define INPUT_SIGNALS                                                                                         \
  m_i_desSIN << m_percentage_dead_zone_compensationSIN << SAFETY_SIGNALS << m_i_max_dead_zone_compensationSIN \
             << m_dqSIN << m_bemf_factorSIN << m_in_out_gainSIN << m_i_measuredSIN << m_dead_zone_offsetsSIN  \
             << m_i_sens_gainsSIN << m_i_sensor_offsets_low_levelSIN << m_i_sensor_offsets_real_inSIN         \
             << m_kp_currentSIN << m_ki_currentSIN << m_percentage_bemf_compensationSIN
#define OUTPUT_SIGNALS                                                                                           \
  m_uSOUT << m_u_safeSOUT << m_i_realSOUT << m_i_low_levelSOUT << m_dead_zone_compensationSOUT << m_i_errorsSOUT \
...
CurrentController::CurrentController(const std::string& name)
    : Entity(name),
      ...
      CONSTRUCT_SIGNAL_OUT(
          u_safe, dynamicgraph::Vector,
          INPUT_SIGNALS << m_uSOUT << m_i_realSOUT << m_i_low_levelSOUT << m_i_sensor_offsets_real_outSOUT),
      CONSTRUCT_SIGNAL_OUT(i_real, dynamicgraph::Vector,
                           m_i_measuredSIN << m_i_sens_gainsSIN << m_i_sensor_offsets_real_outSOUT),
      CONSTRUCT_SIGNAL_OUT(i_low_level, dynamicgraph::Vector,
                           m_i_measuredSIN << m_i_sens_gainsSIN << m_i_sensor_offsets_low_levelSIN),
      CONSTRUCT_SIGNAL_OUT(i_sensor_offsets_real_out, dynamicgraph::Vector,
                           m_i_measuredSIN << m_i_sensor_offsets_real_inSIN),
      CONSTRUCT_SIGNAL_OUT(dead_zone_compensation, dynamicgraph::Vector, m_u_safeSOUT << m_dead_zone_offsetsSIN),
      CONSTRUCT_SIGNAL_OUT(i_errors, dynamicgraph::Vector, m_i_realSOUT << m_uSOUT),
      CONSTRUCT_SIGNAL_OUT(
          i_errors_ll_wo_bemf, dynamicgraph::Vector,
          m_i_realSOUT << m_uSOUT << m_percentage_bemf_compensationSIN << m_dqSIN << m_bemf_factorSIN),
  ...
  addCommand("init", makeCommandVoid3(
                         *this, &CurrentController::init,
                         docCommandVoid3(
                             "Initialize the entity.", "Time period in seconds (double)", "Robot reference (string)",
                             "Number of iterations while control is disabled to calibrate current sensors (int)")));

  addCommand("reset_integral", makeCommandVoid0(*this, &CurrentController::reset_integral,
                                                docCommandVoid0("Reset the integral error.")));
}

void CurrentController::init(const double& dt, const std::string& robotRef, const unsigned int& currentOffsetIters) {
...
DEFINE_SIGNAL_OUT_FUNCTION(u, dynamicgraph::Vector) {
  ...
  if (m_emergency_stop_triggered || m_iter < static_cast<int>(m_currentOffsetIters)) s.setZero();
  ...
  for (unsigned int i = 0; i < m_robot_util->m_nbJoints; i++) {
    ...
    if ((fabs(i_real(i)) > i_max)) {
      ...
      SEND_MSG("Joint " + m_robot_util->get_name_from_id(i) +
                   " measured current is too large: " + toString(i_real(i)) + "A > " + toString(i_max) + "A",
               MSG_TYPE_ERROR);
    }

    if (fabs(u(i)) > u_max(i)) {
      m_emergency_stop_triggered = true;
      SEND_MSG("Joint " + m_robot_util->get_name_from_id(i) + " control is too large: " + toString(u(i)) + "A > " +
                   toString(u_max(i)) + "A",
               MSG_TYPE_ERROR);
    }
...
DEFINE_SIGNAL_OUT_FUNCTION(i_sensor_offsets_real_out, dynamicgraph::Vector) {
  ...
  if (m_currentOffsetIters > 0) {
    if (m_iter < static_cast<int>(m_currentOffsetIters))
      ...
      SEND_MSG("Current sensor offsets computed in " + toString(m_iter) + " iterations: " + toString(m_i_offsets_real),
               MSG_TYPE_INFO);
      for (int i = 0; i < s.size(); i++)
        if (fabs(m_i_offsets_real(i)) > 0.6) {
          SEND_MSG("Current offset for joint " + m_robot_util->get_name_from_id(i) +
                       " is too large, suggesting that the sensor may be broken: " + toString(m_i_offsets_real(i)),
                   MSG_TYPE_WARNING);
