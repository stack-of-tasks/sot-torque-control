...
SE3TrajectoryGenerator::SE3TrajectoryGenerator(const std::string& name)
  ...
  /* Commands. */
  addCommand("init", makeCommandVoid1(*this, &SE3TrajectoryGenerator::init,
                                      docCommandVoid1("Initialize the entity.", "Time period in seconds (double)")));
  ...
  addCommand("setSpline",
             makeCommandVoid3(*this, &SE3TrajectoryGenerator::setSpline,
                              docCommandVoid3("Load serialized spline from file", "(string)   filename",
                                              "(double) time to initial conf", "(Matrix) orientation of the point")));

  addCommand("startSinusoid", makeCommandVoid3(*this, &SE3TrajectoryGenerator::startSinusoid,
  ...
  addCommand("startTriangle",
             makeCommandVoid4(*this, &SE3TrajectoryGenerator::startTriangle,
                              docCommandVoid4("Start an infinite triangle wave.", "(int)    index",
                                              "(double) final values", "(double) time to reach the final value in sec",
...
DEFINE_SIGNAL_OUT_FUNCTION(x, dynamicgraph::Vector) {
  ...
  getProfiler().start(PROFILE_SE3_POSITION_DESIRED_COMPUTATION);
  {
    ...
    if (m_firstIter) {
      ...
      for (unsigned int i = 0; i < m_np; i++) m_currentTrajGen[i]->set_initial_point(initial_value(i));
      m_firstIter = false;
    } else if (iter == static_cast<int>(m_iterLast)) {
      if (m_triggerSIN(iter) == true && m_splineReady) startSpline();
      if (m_status[0] == TG_TEXT_FILE) {
        for (unsigned int i = 0; i < m_np; i++) s(i) = m_textFileTrajGen->getPos()[i];
        ...
        for (unsigned int i = 0; i < m_np; i++) s(i) = m_currentTrajGen[i]->getPos()(0);
      getProfiler().stop(PROFILE_SE3_POSITION_DESIRED_COMPUTATION);
      return s;
    }
    m_iterLast = iter;
...
DEFINE_SIGNAL_OUT_FUNCTION(dx, dynamicgraph::Vector) {
  ...
  if (m_status[0] == TG_TEXT_FILE) {
    for (unsigned int i = 0; i < m_nv; i++) s(i) = m_textFileTrajGen->getVel()[i];
  } else if (m_status[0] == TG_SPLINE) {
    const Eigen::Vector3d& t = m_splineTrajGen->derivate(m_t, 1);
    s.segment<3>(0) = t;
    s.segment<3>(3).setZero();
  } else
    for (unsigned int i = 0; i < m_nv; i++) s(i) = m_currentTrajGen[i]->getVel()(0);
...
DEFINE_SIGNAL_OUT_FUNCTION(ddx, dynamicgraph::Vector) {
  ...
  if (m_status[0] == TG_TEXT_FILE) {
    for (unsigned int i = 0; i < m_nv; i++) s(i) = m_textFileTrajGen->getAcc()[i];
  } else if (m_status[0] == TG_SPLINE) {
    const Eigen::Vector3d& t = m_splineTrajGen->derivate(m_t, 2);
    s.segment<3>(0) = t;
    s.segment<3>(3).setZero();
  } else
    for (unsigned int i = 0; i < m_nv; i++) s(i) = m_currentTrajGen[i]->getAcc()(0);
...
void SE3TrajectoryGenerator::getValue(const int& id) {
  if (id < 0 || id >= static_cast<int>(m_np)) return SEND_MSG("Index is out of bounds", MSG_TYPE_ERROR);

  SEND_MSG("Current value of component " + toString(id) + " is " + toString(m_currentTrajGen[id]->getPos()(0)),
           MSG_TYPE_INFO);
}
...
void SE3TrajectoryGenerator::setSpline(const std::string& fileName, const double& timeToInitConf,
...
void SE3TrajectoryGenerator::startTriangle(const int& id, const double& xFinal, const double& time,
                                           const double& Tacc) {
...
void SE3TrajectoryGenerator::startLinearChirp(const int& id, const double& xFinal, const double& f0, const double& f1,
                                              const double& time) {
