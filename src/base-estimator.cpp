...
void se3Interp(const pinocchio::SE3& s1, const pinocchio::SE3& s2, const double alpha, pinocchio::SE3& s12) {
  const Eigen::Vector3d t_(s1.translation() * alpha + s2.translation() * (1 - alpha));

  const Eigen::Vector3d w(pinocchio::log3(s1.rotation()) * alpha + pinocchio::log3(s2.rotation()) * (1 - alpha));
...
void rpyToMatrix(const Eigen::Vector3d& rpy, Eigen::Matrix3d& R) { return rpyToMatrix(rpy[0], rpy[1], rpy[2], R); }
...
#define INPUT_SIGNALS                                                                                                \
  m_joint_positionsSIN << m_joint_velocitiesSIN << m_imu_quaternionSIN << m_forceLLEGSIN << m_forceRLEGSIN           \
                       << m_dforceLLEGSIN << m_dforceRLEGSIN << m_w_lf_inSIN << m_w_rf_inSIN << m_K_fb_feet_posesSIN \
                       << m_lf_ref_xyzquatSIN << m_rf_ref_xyzquatSIN << m_accelerometerSIN << m_gyroscopeSIN
#define OUTPUT_SIGNALS                                                                                             \
  m_qSOUT << m_vSOUT << m_v_kinSOUT << m_v_flexSOUT << m_v_imuSOUT << m_v_gyrSOUT << m_lf_xyzquatSOUT              \
          << m_rf_xyzquatSOUT << m_a_acSOUT << m_v_acSOUT << m_q_lfSOUT << m_q_rfSOUT << m_q_imuSOUT << m_w_lfSOUT \
...
BaseEstimator::BaseEstimator(const std::string& name)
    : Entity(name),
      ...
      CONSTRUCT_SIGNAL_OUT(q, dynamicgraph::Vector,
                           m_kinematics_computationsSINNER
                               << m_joint_positionsSIN << m_imu_quaternionSIN << m_forceLLEGSIN << m_forceRLEGSIN
                               << m_w_lf_inSIN << m_w_rf_inSIN << m_K_fb_feet_posesSIN << m_w_lf_filteredSOUT
                               << m_w_rf_filteredSOUT << m_lf_ref_xyzquatSIN << m_rf_ref_xyzquatSIN),
      CONSTRUCT_SIGNAL_OUT(v, dynamicgraph::Vector,
                           m_kinematics_computationsSINNER << m_accelerometerSIN << m_gyroscopeSIN << m_qSOUT
                                                           << m_dforceLLEGSIN << m_dforceRLEGSIN),
  ...
  addCommand("set_fz_stable_windows_size",
             makeCommandVoid1(*this, &BaseEstimator::set_fz_stable_windows_size,
                              docCommandVoid1("Set the windows size used to detect that feet is stable", "int")));
  addCommand("set_alpha_w_filter",
             makeCommandVoid1(*this, &BaseEstimator::set_alpha_w_filter,
                              docCommandVoid1("Set the filter parameter to filter weights", "double")));
  addCommand(
      "set_alpha_DC_acc",
      makeCommandVoid1(*this, &BaseEstimator::set_alpha_DC_acc,
                       docCommandVoid1("Set the filter parameter for removing DC from accelerometer data", "double")));
             ...
             makeCommandVoid1(
                 *this, &BaseEstimator::set_alpha_DC_vel,
                 docCommandVoid1("Set the filter parameter for removing DC from velocity integrated from acceleration",
                                 "double")));
  addCommand("reset_foot_positions", makeCommandVoid0(*this, &BaseEstimator::reset_foot_positions,
                                                      docCommandVoid0("Reset the position of the feet.")));
  addCommand("get_imu_weight",
             makeDirectGetter(*this, &m_w_imu,
                              docDirectGetter("Weight of imu-based orientation in sensor fusion", "double")));
  addCommand("set_imu_weight",
             makeCommandVoid1(*this, &BaseEstimator::set_imu_weight,
                              docCommandVoid1("Set the weight of imu-based orientation in sensor fusion", "double")));
  addCommand(
      "set_zmp_std_dev_right_foot",
      makeCommandVoid1(
          *this, &BaseEstimator::set_zmp_std_dev_right_foot,
          docCommandVoid1("Set the standard deviation of the ZMP measurement errors for the right foot", "double")));
  addCommand(
      "set_zmp_std_dev_left_foot",
      makeCommandVoid1(
          *this, &BaseEstimator::set_zmp_std_dev_left_foot,
          docCommandVoid1("Set the standard deviation of the ZMP measurement errors for the left foot", "double")));
  addCommand(
  ...
  addCommand("set_stiffness_right_foot",
             makeCommandVoid1(*this, &BaseEstimator::set_stiffness_right_foot,
                              docCommandVoid1("Set the 6d stiffness of the spring at the right foot", "vector")));
  addCommand("set_stiffness_left_foot",
             makeCommandVoid1(*this, &BaseEstimator::set_stiffness_left_foot,
                              docCommandVoid1("Set the 6d stiffness of the spring at the left foot", "vector")));
  addCommand(
  ...
  addCommand("set_normal_force_margin_right_foot",
             makeCommandVoid1(*this, &BaseEstimator::set_normal_force_margin_right_foot,
                              docCommandVoid1("Set the normal force margin for the right foot", "double")));
  addCommand("set_normal_force_margin_left_foot",
             makeCommandVoid1(*this, &BaseEstimator::set_normal_force_margin_left_foot,
                              docCommandVoid1("Set the normal force margin for the left foot", "double")));
}
...
void BaseEstimator::set_fz_stable_windows_size(const int& ws) {
  if (ws < 0.0) return SEND_MSG("windows_size should be a positive integer", MSG_TYPE_ERROR);
  m_fz_stable_windows_size = ws;
}

void BaseEstimator::set_alpha_w_filter(const double& a) {
  if (a < 0.0 || a > 1.0) return SEND_MSG("alpha should be in [0..1]", MSG_TYPE_ERROR);
  m_alpha_w_filter = a;
}
...
void BaseEstimator::set_imu_weight(const double& w) {
  if (w < 0.0) return SEND_MSG("Imu weight must be nonnegative", MSG_TYPE_ERROR);
  m_w_imu = w;
}

void BaseEstimator::set_stiffness_right_foot(const dynamicgraph::Vector& k) {
  if (k.size() != 6) return SEND_MSG("Stiffness vector should have size 6, not " + toString(k.size()), MSG_TYPE_ERROR);
  m_K_rf = k;
}
...
void BaseEstimator::set_zmp_std_dev_right_foot(const double& std_dev) {
  if (std_dev <= 0.0) return SEND_MSG("Standard deviation must be a positive number", MSG_TYPE_ERROR);
...
void BaseEstimator::set_zmp_margin_right_foot(const double& margin) { m_zmp_margin_rf = margin; }

void BaseEstimator::set_zmp_margin_left_foot(const double& margin) { m_zmp_margin_lf = margin; }

void BaseEstimator::set_normal_force_margin_right_foot(const double& margin) { m_fz_margin_rf = margin; }

void BaseEstimator::set_normal_force_margin_left_foot(const double& margin) { m_fz_margin_lf = margin; }
...
double BaseEstimator::compute_zmp_weight(const Vector2& zmp, const Vector4& foot_sizes, double std_dev,
                                         double margin) {
  ...
  double cdx = ((cdf(m_normal, (fs0 - zmp[0]) / std_dev) - cdf(m_normal, (fs1 - zmp[0]) / std_dev)) - 0.5) * 2.0;
  double cdy = ((cdf(m_normal, (fs2 - zmp[1]) / std_dev) - cdf(m_normal, (fs3 - zmp[1]) / std_dev)) - 0.5) * 2.0;
  return cdx * cdy;
}
...
void BaseEstimator::reset_foot_positions_impl(const Vector6& ftlf, const Vector6& ftrf) {
  ...
  kinematics_estimation(ftrf, m_K_rf, m_oMrfs, static_cast<int>(m_right_foot_id), rfMff, dummy,
                        dummy1);  // rfMff is obtain reading oMff becaused oMrfs is here set to Identity
  kinematics_estimation(ftlf, m_K_lf, m_oMlfs, static_cast<int>(m_left_foot_id), lfMff, dummy, dummy1);
...
void BaseEstimator::kinematics_estimation(const Vector6& ft, const Vector6& K, const SE3& oMfs, const int foot_id,
...
DEFINE_SIGNAL_INNER_FUNCTION(kinematics_computations, dynamicgraph::Vector) {
  ...
  assert(qj.size() == static_cast<Eigen::VectorXd::Index>(m_robot_util->m_nbJoints) &&
...
DEFINE_SIGNAL_OUT_FUNCTION(q, dynamicgraph::Vector) {
  ...
  if (wR == 0.0 && wL == 0.0) {
    SEND_WARNING_STREAM_MSG("The robot is flying!" + ("- forceRLEG: " + toString(ftrf.transpose())) +
  ...
  getProfiler().start(PROFILE_BASE_POSITION_ESTIMATION);
  {
    ...
    Vector3 rpy_chest, rpy_chest_lf, rpy_chest_rf,
        rpy_chest_imu;  // orientation of the body which imu is attached to. (fusion, from left kine, from right kine,
                        // from imu)
    ...
    m_q_pin.head<3>() = ((oMlfa.translation() - pos_lf_ff) * wL + (oMrfa.translation() - pos_rf_ff) * wR) / (wL + wR);
    ...
    if (m_K_fb_feet_posesSIN.isPlugged()) {
      ...
      if (K_fb > 0.0 && m_w_imu > 0.0) {
        ...
        if (m_lf_ref_xyzquatSIN.isPlugged() and m_rf_ref_xyzquatSIN.isPlugged()) {
          ...
          const Eigen::Quaterniond ql(m_lf_ref_xyzquatSIN(iter)(3), m_lf_ref_xyzquatSIN(iter)(4),
                                      m_lf_ref_xyzquatSIN(iter)(5), m_lf_ref_xyzquatSIN(iter)(6));
          const Eigen::Quaterniond qr(m_rf_ref_xyzquatSIN(iter)(3), m_rf_ref_xyzquatSIN(iter)(4),
        ...
        const double yaw_drift = (atan2(V_ref(1), V_ref(0)) - atan2(V_est(1), V_est(0)));
        //~ printf("yaw_drift=%lf\r\n",yaw_drift);
...
DEFINE_SIGNAL_OUT_FUNCTION(w_lf, double) {
  ...
  double w_zmp = compute_zmp_weight(zmp, m_left_foot_sizes, m_zmp_std_dev_lf, m_zmp_margin_lf);
  double w_fz = compute_force_weight(wrench(2), m_fz_std_dev_lf, m_fz_margin_lf);
  // check that foot is sensing a force greater than the margin treshold for more than 'm_fz_stable_windows_size'
  // samples
...
DEFINE_SIGNAL_OUT_FUNCTION(w_rf, double) {
  ...
  zmp.setZero();
  compute_zmp(wrench, zmp);
  double w_zmp = compute_zmp_weight(zmp, m_right_foot_sizes, m_zmp_std_dev_rf, m_zmp_margin_rf);
...
DEFINE_SIGNAL_OUT_FUNCTION(v, dynamicgraph::Vector) {
  ...
  getProfiler().start(PROFILE_BASE_VELOCITY_ESTIMATION);
  {
    ...
    Vector6 v_flex_l;
    Vector6 v_flex_r;
    v_flex_l << -dftlf[0] / m_K_lf(0), -dftlf[1] / m_K_lf(1), -dftlf[2] / m_K_lf(2), -dftlf[3] / m_K_lf(3),
        -dftlf[4] / m_K_lf(4), -dftlf[5] / m_K_lf(5);
    v_flex_r << -dftrf[0] / m_K_rf(0), -dftrf[1] / m_K_rf(1), -dftrf[2] / m_K_rf(2), -dftrf[3] / m_K_rf(3),
        -dftrf[4] / m_K_rf(4), -dftrf[5] / m_K_rf(5);
    ...
    // we make the asumtion than we are 'turning around the foot' with pure angular velocity in the ankle measured by
    // the gyro
    ...
    const SE3 chestMimu(
        ...
        Vector3(
            -0.13, 0.0,
            0.118));  /// TODO Read this transform from setable parameter /// TODO chesk the sign of the translation
    ...
    Vector3 gVo_a_l = Vector3(gyr_imu(0), gyr_imu(1), gyr_imu(2)) + (imuMff * ffMlf).act(v_lf_local).angular() -
                      m_data->v[f_imu.parent].angular();
    Vector3 gVo_a_r = Vector3(gyr_imu(0), gyr_imu(1), gyr_imu(2)) + (imuMff * ffMrf).act(v_rf_local).angular() -
                      m_data->v[f_imu.parent].angular();
    ...
    // const Frame & f_imu = m_model.frames[m_IMU_body_id];
    // const SE3 ffMchest(m_data->oMf[m_IMU_body_id]);
    // const SE3 chestMimu(Matrix3::Identity(), +1.0*Vector3(-0.13, 0.0,  0.118)); ///TODO Read this transform from
    // setable parameter /// TODO chesk the sign of the translation
