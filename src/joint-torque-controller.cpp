...
#define MODEL_INPUT_SIGNALS                                                                                \
  m_motorParameterKt_pSIN << m_motorParameterKt_nSIN << m_motorParameterKf_pSIN << m_motorParameterKf_nSIN \
                          << m_motorParameterKv_pSIN << m_motorParameterKv_nSIN << m_motorParameterKa_pSIN \
                          << m_motorParameterKa_nSIN << m_polySignDqSIN

#define ESTIMATOR_INPUT_SIGNALS                                                                   \
  m_jointsPositionsSIN << m_jointsVelocitiesSIN << m_jointsAccelerationsSIN << m_jointsTorquesSIN \
                       << m_jointsTorquesDerivativeSIN

#define TORQUE_INTEGRAL_INPUT_SIGNALS m_KiTorqueSIN << m_torque_integral_saturationSIN
#define TORQUE_CONTROL_INPUT_SIGNALS \
  m_jointsTorquesDesiredSIN << m_KpTorqueSIN << m_KdTorqueSIN << m_coulomb_friction_compensation_percentageSIN
#define VEL_CONTROL_INPUT_SIGNALS m_dq_desSIN << m_KdVelSIN << m_KiVelSIN

#define ALL_INPUT_SIGNALS                                                                  \
  ESTIMATOR_INPUT_SIGNALS << TORQUE_INTEGRAL_INPUT_SIGNALS << TORQUE_CONTROL_INPUT_SIGNALS \
...
JointTorqueController::JointTorqueController(const std::string& name)
    : Entity(name),
      ...
      CONSTRUCT_SIGNAL_OUT(u, dynamicgraph::Vector,
                           ESTIMATOR_INPUT_SIGNALS << TORQUE_CONTROL_INPUT_SIGNALS << VEL_CONTROL_INPUT_SIGNALS
      ...
      CONSTRUCT_SIGNAL_OUT(torque_error_integral, dynamicgraph::Vector,
                           m_jointsTorquesSIN << m_jointsTorquesDesiredSIN << TORQUE_INTEGRAL_INPUT_SIGNALS) {
  Entity::signalRegistration(ALL_INPUT_SIGNALS << ALL_OUTPUT_SIGNALS);

  /* Commands. */
  addCommand("getTimestep", makeDirectGetter(*this, &m_dt, docDirectGetter("Control timestep", "double")));

  addCommand("init", makeCommandVoid2(*this, &JointTorqueController::init,
                                      docCommandVoid2("Initialize the controller.", "Control timestep [s].",
...
/* --- COMMANDS ---------------------------------------------------------- */
/* --- COMMANDS ---------------------------------------------------------- */
void JointTorqueController::init(const double& timestep, const std::string& robot_ref) {
  ...
  if (!m_KpTorqueSIN.isPlugged()) return SEND_MSG("Init failed: signal m_KpTorqueSIN is not plugged", MSG_TYPE_ERROR);
  if (!m_KiTorqueSIN.isPlugged()) return SEND_MSG("Init failed: signal m_KiTorqueSIN is not plugged", MSG_TYPE_ERROR);
...
DEFINE_SIGNAL_OUT_FUNCTION(u, dynamicgraph::Vector) {
  ...
  if (saturating) SEND_INFO_STREAM_MSG("Saturate dqErr integral: " + toString(m_dqErrIntegral.head<12>()));
  ...
  for (int i = 0; i < (int)m_robot_util->m_nbJoints; i++) {
    m_current_des(i) = motorModel.getCurrent(
        ...
        ddq(i + offset), motorParameterKt_p(i), motorParameterKt_n(i), motorParameterKf_p(i) * colFricCompPerc(i),
        motorParameterKf_n(i) * colFricCompPerc(i), motorParameterKv_p(i), motorParameterKv_n(i),
        motorParameterKa_p(i), motorParameterKa_n(i), static_cast<unsigned int>(polySignDq(i)));
  }
...
DEFINE_SIGNAL_OUT_FUNCTION(smoothSignDq, dynamicgraph::Vector) {
  const Eigen::VectorXd& dq = m_jointsVelocitiesSIN(iter);
  const Eigen::VectorXd& polySignDq = m_polySignDqSIN(iter);
  if (s.size() != (int)m_robot_util->m_nbJoints) s.resize(m_robot_util->m_nbJoints);

  for (int i = 0; i < (int)m_robot_util->m_nbJoints; i++)
    s(i) = motorModel.smoothSign(dq[i], 0.1, static_cast<unsigned int>(polySignDq[i]));
