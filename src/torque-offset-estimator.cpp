...
TorqueOffsetEstimator::TorqueOffsetEstimator(const std::string& name)
  ...
  addCommand("init", makeCommandVoid5(*this, &TorqueOffsetEstimator::init,
                                      docCommandVoid5("Initialize the estimator", "urdfFilePath",
  ...
  addCommand("computeOffset",
             makeCommandVoid2(*this, &TorqueOffsetEstimator::computeOffset,
                              docCommandVoid2("Compute the offset for sensor calibration",
                                              "Number of iteration to average over.", "Maximum allowed offset")));
  addCommand("getSensorOffsets", makeDirectGetter(*this, &jointTorqueOffsets,
...
/* --- COMMANDS ---------------------------------------------------------- */
/* --- COMMANDS ---------------------------------------------------------- */
void TorqueOffsetEstimator::init(const std::string& robotRef, const Eigen::Matrix4d& m_torso_X_imu_,
                                 const double& gyro_epsilon_, const std::string& ffJointName,
  ...
  if (sensor_offset_status == INPROGRESS) {
    ...
    if (i < n_iterations) {
      ...
      const Eigen::VectorXd& tau_rnea =
          pinocchio::rnea(m_model, *m_data, enc, Eigen::VectorXd::Zero(m_model.nv), Eigen::VectorXd::Zero(m_model.nv));
