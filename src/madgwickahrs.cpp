...
#define OUTPUT_SIGNALS    m_imu_quatSOUT
      ...
      MadgwickAHRS::MadgwickAHRS(const std::string& name)
            ...
            ,CONSTRUCT_SIGNAL(imu_quat, OUT,                ml::Vector)
            ,m_initSucceeded(false)
            ,m_beta(betaDef)
      {
        BIND_SIGNAL_TO_FUNCTION(imu_quat,   OUT, ml::Vector);
      ...
      DEFINE_SIGNAL_OUT_FUNCTION(imu_quat, ml::Vector)
      {
        if(!m_initSucceeded)
        {
        ...
        const ml::Vector& accelerometer = m_accelerometerSIN(iter);
        const ml::Vector& gyroscope = m_gyroscopeSIN(iter);
