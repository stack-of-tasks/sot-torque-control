...
def homo16toSE3(M):
    r = np.matrix([[M[0], M[1], M[2]], [M[4], M[5], M[6]], [M[8], M[9], M[10]]])
...
def homoToXYZRPY(H):
    r = np.matrix([[H[0], H[1], H[2]], [H[4], H[5], H[6]], [H[8], H[9], H[10]]])
...
def rpyDerivativeToAngVel(rpy, drpy):
    ...
    T = np.matrix([[cos(p) * cos(y), -sin(y), 0], [cos(p) * sin(y), cos(y), 0], [-sin(p), 0, 1]])
    # T = np.matrix([ [1., 0., 0 ] ,
    # [0., 1., 0 ] ,
    # [0., 0 , 1 ]]);
