...
if(PLOT_3D_SURFACE):
    ...
    plot3d(dq, tau, ctrl, 'Asymmetric ID - '+data_folder[-10:], ['dq','tau','delta q'],'r ', ax=ax);
    ax.plot_wireframe(X,Y,Z);
    ax = fig.add_subplot(222, projection='3d');
    Z = k_v_ls*X + k_tau_ls*Y;
    plot3d(dq, tau, ctrl, 'Symmetric ID - '+data_folder[-10:], ['dq','tau','delta q'],'r ', ax=ax);
    ax.plot_wireframe(X,Y,Z);
    ax = fig.add_subplot(223, projection='3d');
    Z = k_v_c1*X + k_tau_c1*Y + k_cp_c1*Xp + k_cn_c1*Xn + k_tp_c1*Yp + k_tn_c1*Yn;
    plot3d(dq, tau, ctrl, 'Symmetric ID with Coloumb friction- '+data_folder[-10:], ['dq','tau','delta q'],'r ', ax=ax);
    ax.plot_wireframe(X,Y,Z);
...
if(PLOT_LOW_VEL_DATA_3D==True):
    ...
    ax = fig.add_subplot(221, projection='3d');
    Z = k_v*X + k_tau*Y;
    plot3d(dq_zero_vel, tau_zero_vel, ctrl_zero_vel, 'Low velocity data asymmetric penalty '+data_folder[-10:], ['dq','tau','delta q'],'r ', ax=ax);
    ax.plot_wireframe(X,Y,Z);
    ax = fig.add_subplot(222, projection='3d');
    Z = k_v_ls*X + k_tau_ls*Y;
    plot3d(dq_zero_vel, tau_zero_vel, ctrl_zero_vel, 'Low velocity data symmetric penalty '+data_folder[-10:], ['dq','tau','delta q'],'r ', ax=ax);
    ax.plot_wireframe(X,Y,Z);
    ax = fig.add_subplot(223, projection='3d');
    Z = k_v_c1*X + k_tau_c1*Y + k_tp_c1*Yp + k_tn_c1*Yn;
    plot3d(dq_zero_vel, tau_zero_vel, ctrl_zero_vel, 'Low velocity data Coloumb '+data_folder[-10:], ['dq','tau','delta q'],'r ', ax=ax);
    ax.plot_wireframe(X,Y,Z);
...
if(PLOT_LOW_VEL_DATA==True):
    ...
    f, ax = plt.subplots(1,1,sharex=True);
    pos_line, = ax.plot(tau_zero_vel[i_dq_pos], ctrl_zero_vel[i_dq_pos]-k_v*dq_zero_vel[i_dq_pos],'r .',rasterized=True);
    neg_line, = ax.plot(tau_zero_vel[i_dq_neg], ctrl_zero_vel[i_dq_neg]-k_v*dq_zero_vel[i_dq_neg],'b .',rasterized=True);
    if(len(i_dq_unc[0])>0):
        ax.plot(tau_zero_vel[i_dq_unc], ctrl_zero_vel[i_dq_unc]-k_v*dq_zero_vel[i_dq_unc],'g .',rasterized=True);
...
if(PLOT_PIECE_WISE_LINEAR):    
    ...
    ctrl_model_pos = np.array([pwl.f_k1p[JOINT_ID]*tau_min              +pwl.f_q1p[JOINT_ID], 
    ...
    ctrl_model_neg = np.array([pwl.f_k1n[JOINT_ID]*tau_min              +pwl.f_q1n[JOINT_ID], 
