...
DEFAULT_AXES_LABEL_SIZE = DEFAULT_FONT_SIZE+25;  # fontsize of the x any y labels
...
def saveCurrentFigure(name, raster=True):
    if(SAVE_FIGURES):
        for ext in FILE_EXTENSIONS:
            plt.gcf().savefig(FIGURE_PATH+name.replace(' ', '_')+'.'+ext, format=ext, dpi=FIGURES_DPI, 
                    bbox_inches='tight', rasterized=raster);
    else:
...
def plotNdQuantityPerSolver(nRows, nCols, quantity, title, solver_names, line_styles, ax=None, boundUp=None, boundLow=None, 
                            yscale='linear', subplot_titles=None, ylabels=None, sharey=False, margins=None):
    if(ax==None):
    ...
    if(SHOW_LEGENDS and solver_names!=None):
