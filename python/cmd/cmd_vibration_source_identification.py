...
smoothly_set_signal(robot.inv_dyn.kp_com, (20.,20.,20.))
smoothly_set_signal(robot.inv_dyn.kd_com, (7.,0.,0.))
...
# !!! Test 1: Baseline
# RESULTS: robot is rather stable, only a bit of jerky motions coming from left knee, but not amplified by control
...
# After doing that, the system should be stable because the only feedback comes from the encoders (which are basically noiseless), the current sensors (which are only used for the dead-zone compensation and the integral feedback) and the joint velocities used for friction and back-EMF compensation (which did not give any vibration with the robot in the air).
# If we still have vibrations with this setup it could potentially be good news because it would mean we can remove them easily, filtering more the current sensors or the joint velocities, but I doubt that'll be the case.
...
This is my main suspect as source of vibrations. However, in earlier experiments we've seen that for sure this is not the only source, because we experienced vibrations even when kp-torque was set to zero.
...
This test investigates the noise of the base estimation. Ithis noise turns out to critical, we can always reduce it by neglecting certain axes of the ft measurements. While we know that the flexibility allows for 6d deformations, we also know that the main deflections happen in roll and pitch, so we could neglect the other directions, thus reducing the noise in the base estimation. If the noise really comes from the roll-pitch measurements, then the only option is to filter more.

#!!! Test 3B: Add Torque Integral
# RESULTS: No vibrations, better tracking, but controlled divergence of CoM. We noticed an offset of about 1 cm in CoM w.r.t. CoP, which may contribute to this effect, so we tried to compensate for it in balance controller. It improved a bit the behavior, but it was still diverging, which is not surprising given the lack of damping. Setting kp-torque to 2 for the ankle pitch joints improved the CoP tracking, but the divergence didn't disappear.
...
This is just to see whether the IMU introduces significant noise in the base estimation. If it does, we can simply stop using it, or try to filter it more.
...
This is just to test the base velocity estimation. If we see vibrations, we can then investigate whether they come form the gyroscope of the encoders. In any case, more filtering is the only solution I see in this case.
