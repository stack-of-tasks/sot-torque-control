...
if IDENTIFICATION_MODE == "static":
    (Ktp, Ktn, Ks, DZ) = identify_motor_static(
        enc,
        dq,
        ctrl,
        current,
...
if IDENTIFICATION_MODE == "vel":
    ...
    (Kvp, Kvn, Kfp, Kfn, DeadZone, K_bemf) = identify_motor_vel(
        dt,
        dq,
        ddq,
        ctrl,
    ...
    print(
        "deadzone[%d]      = %f #Using %s"
        % (
            JOINT_ID,
            data_motor_param["DZ"].item(),
            data_folder_static.split("/")[-2] + warning,
        )
    )
    print("deadzone[%d]      = %f #Using %s" % (JOINT_ID, DeadZone, data_folder_vel.split("/")[-2] + warning))
    print(
        "K_bemf[%d]        = %f # [Amp/Rad.s-1] Using %s"
...
if IDENTIFICATION_MODE == "acc":
    ...
    (Kap, Kan, Kfp, Kfn) = identify_motor_acc(
        dt,
        dq,
        ddq,
        current,
