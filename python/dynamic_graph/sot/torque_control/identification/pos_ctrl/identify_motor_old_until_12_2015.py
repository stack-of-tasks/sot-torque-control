...
if (FOLDER_ID == 1):
    ...
    ZERO_VEL_THR = 0.1
      ...
      ):  # data collected with pos ctrl (me pushing) changing the configuration of the leg during the experiment
    data_folder = '../results/20150424_112301_rhp_torque_id_multi_conf/'
    JOINT_ID = 2
    DATA_FILE_NAME = 'data_j' + str(JOINT_ID) + '.npz'
elif (FOLDER_ID == 19
      ):  # data collected with pos ctrl (me pushing) changing the normal force on the right foot during the experiment
...
if (DO_IDENTIFICATION_WITH_FIRST_ORDER_DYNAMICS):
    ...
    print 'k_v[%d] = %f\nk_tau[%d] = %f\tk_tau[%d] = %f' % (JOINT_ID, k_v_dyn, JOINT_ID, k_tau1_dyn, JOINT_ID,
                                                            k_tau2_dyn)
...
# solution of the real problem
print 'k_v[%d] = %f\nk_tau[%d] = %f\nk_cp[%d] = %f\nk_cn[%d] = %f' % (JOINT_ID, k_v_c1, JOINT_ID, k_tau_c1, JOINT_ID,
                                                                      k_cp_c1, JOINT_ID, k_cn_c1)
...
if (PLOT_3D_SURFACE):
    ...
    plot3d(dq,
           tau,
           delta_q,
           'Symmetric ID with Coloumb friction- ' + data_folder, ['dq', 'tau', 'delta q'],
           'r ',
           ax=ax)
    ax.plot_wireframe(X, Y, Z)
...
if (PLOT_LOW_VEL_DATA_3D == True):
    ...
    plot3d(dq_zero_vel,
           tau_zero_vel,
           delta_q_zero_vel,
           'Low velocity data asymmetric penalty ' + data_folder, ['dq', 'tau', 'delta q'],
           'r ',
           ax=ax)
    ax.plot_wireframe(X, Y, Z)
    ax = fig.add_subplot(222, projection='3d')
    ...
    plot3d(dq_zero_vel,
           tau_zero_vel,
           delta_q_zero_vel,
           'Low velocity data symmetric penalty ' + data_folder, ['dq', 'tau', 'delta q'],
           'r ',
           ax=ax)
    ax.plot_wireframe(X, Y, Z)
    ax = fig.add_subplot(223, projection='3d')
    ...
    plot3d(dq_zero_vel,
           tau_zero_vel,
           delta_q_zero_vel,
           'Low velocity data Coloumb ' + data_folder, ['dq', 'tau', 'delta q'],
           'r ',
           ax=ax)
    ax.plot_wireframe(X, Y, Z)
...
if (PLOT_LOW_VEL_DATA == True):
    ...
    f, ax = plt.subplots(1, 1, sharex=True)
    pos_line, = ax.plot(tau_zero_vel[i_dq_pos],
                        delta_q_zero_vel[i_dq_pos] - k_v * dq_zero_vel[i_dq_pos],
                        'r .',
                        rasterized=True)
    neg_line, = ax.plot(tau_zero_vel[i_dq_neg],
                        delta_q_zero_vel[i_dq_neg] - k_v * dq_zero_vel[i_dq_neg],
                        'b .',
                        rasterized=True)
    if (len(i_dq_unc[0]) > 0):
        ax.plot(tau_zero_vel[i_dq_unc],
                delta_q_zero_vel[i_dq_unc] - k_v * dq_zero_vel[i_dq_unc],
                'g .',
                rasterized=True)
        leg = ax.legend(['Positive velocity', 'Negative velocity', 'Uncertain velocity'], numpoints=1, markerscale=5)
    else:
        leg = ax.legend([pos_line, neg_line], ['Positive velocity', 'Negative velocity'],
                        loc='lower right',
                        numpoints=1,
                        markerscale=5)
