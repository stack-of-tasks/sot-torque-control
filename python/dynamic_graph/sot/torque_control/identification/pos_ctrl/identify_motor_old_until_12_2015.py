...
if PLOT_3D_SURFACE:
    ...
    plot3d(
        dq, tau, delta_q, "Symmetric ID with Coloumb friction- " + data_folder, ["dq", "tau", "delta q"], "r ", ax=ax
    )
    ax.plot_wireframe(X, Y, Z)
...
if PLOT_LOW_VEL_DATA_3D == True:
    ...
    plot3d(
        ...
        tau_zero_vel,
        delta_q_zero_vel,
        "Low velocity data asymmetric penalty " + data_folder,
        ["dq", "tau", "delta q"],
        "r ",
        ax=ax,
    )
    ...
    plot3d(
        ...
        tau_zero_vel,
        delta_q_zero_vel,
        "Low velocity data symmetric penalty " + data_folder,
        ["dq", "tau", "delta q"],
        "r ",
        ax=ax,
    )
    ...
    plot3d(
        ...
        tau_zero_vel,
        delta_q_zero_vel,
        "Low velocity data Coloumb " + data_folder,
        ["dq", "tau", "delta q"],
        "r ",
        ax=ax,
    )
...
if PLOT_LOW_VEL_DATA == True:
    ...
    f, ax = plt.subplots(1, 1, sharex=True)
    (pos_line,) = ax.plot(
        tau_zero_vel[i_dq_pos], delta_q_zero_vel[i_dq_pos] - k_v * dq_zero_vel[i_dq_pos], "r .", rasterized=True
    )
    (neg_line,) = ax.plot(
        tau_zero_vel[i_dq_neg], delta_q_zero_vel[i_dq_neg] - k_v * dq_zero_vel[i_dq_neg], "b .", rasterized=True
    )
    if len(i_dq_unc[0]) > 0:
        ax.plot(
            tau_zero_vel[i_dq_unc], delta_q_zero_vel[i_dq_unc] - k_v * dq_zero_vel[i_dq_unc], "g .", rasterized=True
        )
