...
if (PLOT_DATA):
    ...
    for i in range(len(JOINT_ID)):
        ...
        plt.figure()
        plt.plot(tau[:, i], qDes[:, i] - enc[:, JOINT_ID[i]])
        plt.title('Torque VS delta_q ' + str(JOINT_ID[i]))
        #        plt.figure(); plt.plot(tau[:,i], currents[:,i]); plt.title('Torque VS current '+str(JOINT_ID[i]));
        plt.figure()
        plt.plot(tau[:, i], ptorques[:, i])
        plt.title('Torque VS pseudo-torque ' + str(JOINT_ID[i]))
        plt.figure()
        plt.plot(ptorques[:, i], qDes[:, i] - enc[:, JOINT_ID[i]])
        plt.title('Pseudo-torque VS delta_q ' + str(JOINT_ID[i]))
