...
EST_DELAY = 50;     ''' delay introduced by the estimation in number of samples '''
DELTA_Q_MAX = 0.02;
...
if(FOLDER_ID==1):
    ...
    JOINT_ID = np.array([2]); # hip pitch chirp with higher delay and no F/T sensor in velocity part
    EST_DELAY = 70;
elif(FOLDER_ID==11):
    ...
    JOINT_ID = np.array([2]); # hip pitch chirp with 0 vel
    EST_DELAY = 100;
elif(FOLDER_ID==14 ):
...
#    plt.figure(); plt.plot(enc[:,i]-q[:,i]); plt.title('encoders - q');
    plt.figure(); plt.plot(enc[:,i]-qDes[:,i]); plt.title('encoders-qDes');
    plt.figure(); plt.plot(dq[:,i]); plt.title('Joint velocity');
    plt.figure(); plt.plot(tau[:,i]); plt.title('Joint torques');
    plt.figure(); plt.plot(tau[:,i]); plt.plot(5*dq[:,i]); plt.plot(1e3*(enc[:,i]-qDes[:,i])); plt.title('tau VS dq VS delta_q');
