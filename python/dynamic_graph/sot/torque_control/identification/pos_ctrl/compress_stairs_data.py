...
#    q       = np.loadtxt(data_folder+file_name_q);
    ...
    np.savez(data_folder+DATA_FILE_NAME, dq=dq, tau=tau, qDes=qDes, enc=enc, qRef=qRef, dqRef=dqRef,
             delta_q_ff=delta_q_ff, delta_q_fb=delta_q_fb);
...
for i in range(len(JOINT_ID)):
    print 'Max position tracking error for joint %d:         %f' % (JOINT_ID[i], np.max(np.abs(enc[:,i]-qRef[:,i])));
    print 'Avg Squared position tracking error for joint %d: %f' % (JOINT_ID[i], np.linalg.norm(enc[:,i]-qRef[:,i])/N);
    
    if(PLOT_TRACKING_ERROR):
        plt.figure(); plt.plot(time, 1e3*(enc[:,i]-qRef[:,i]),rasterized=True); 
        plt.xlabel('Time [s]');
    ...
    if(PLOT_TRAJECTORY_TRACKING):        
        plt.figure(); plt.plot(time, enc[:,i]); plt.plot(time, qRef[:,i], 'r--',rasterized=True); 
        plt.xlabel('Time [s]');
