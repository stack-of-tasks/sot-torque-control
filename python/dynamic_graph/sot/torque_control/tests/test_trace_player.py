...
DATA_FILE_NAMES = {"accelerometer": "dg_HRP2LAAS-accelerometer.dat",
...
for signalName in DATA_FILE_NAMES:
    player.addOutputSignal(DATA_PATH+DATA_FILE_NAMES[signalName],
                           signalName);

# CREATE EMPTY NUMPY ARRAYS TO CONTAIN DATA
data = {};
player.trigger.recompute(0);    # alternatively you can also use player.playNext();
for signalName in DATA_FILE_NAMES:
...
for t in range(N):
    if(t==RESET_TIME):
        player.rewind();
    player.trigger.recompute(t);    # alternatively you can also use player.playNext();
