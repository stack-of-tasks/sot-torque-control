...
DEFAULT_FONT_SERIF = [
    'Times New Roman', 'Times', 'Bitstream Vera Serif', 'DejaVu Serif', 'New Century Schoolbook',
...
DEFAULT_AXES_LABEL_SIZE = DEFAULT_FONT_SIZE + 25
...
def create_empty_figure(nRows=1, nCols=1, spinesPos=None):
...
def plot3d(x, y, z, title='', labels=['', '', ''], linestyle='k-', marker='.', ax=None):
    if (ax is None):
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
    ax.plot(x, y, z, linestyle, marker=marker)
    ax.set_title(title)
...
def saveCurrentFigure(name, raster=True):
    if (SAVE_FIGURES):
        for ext in FILE_EXTENSIONS:
            plt.gcf().savefig(FIGURE_PATH + name.replace(' ', '_') + '.' + ext,
                              format=ext,
                              dpi=FIGURES_DPI,
                              bbox_inches='tight',
                              rasterized=raster)
    else:
        print("Not saving figure " + name)


def plot_x_vs_y(x, y, title='', linestyle='k-', lw=DEFAULT_LINE_WIDTH):
    f, ax = plt.subplots(1, 1)
    ax.set_title(title)
    ax.plot(x, y, linestyle, alpha=LINE_ALPHA, linewidth=lw)
    if (SAVE_FIGURES):
        for ext in FILE_EXTENSIONS:
            plt.gcf().savefig(FIGURE_PATH + title.replace(' ', '_') + '.' + ext,
                              format=ext,
                              dpi=FIGURES_DPI,
                              bbox_inches='tight')
    return ax
...
def plotNdQuantity(nRows,
    ...
    return plotNdQuantityPerSolver(nRows, nCols, quantity.reshape(t, 1, n), title, None, [linestyle], ax, boundUp,
                                   boundLow, yscale, None, None, sharey, margins)
...
def plotNdQuantityPerSolver(nRows,
            ...
            if (k < quantity.shape[2]):
                ...
                if (boundUp is not None):
                    if (len(boundUp.shape) == 1):  # constant bound
                        ...
                        ax[i, j].plot([0, quantity.shape[0] - 1], [boundUp[k], boundUp[k]],
                                      '--',
                                      color=BOUNDS_COLOR,
                                      alpha=LINE_ALPHA)
                ...
                if (boundLow is not None):
                    if (len(boundLow.shape) == 1):
                        ...
                        ax[i, j].plot([0, quantity.shape[0] - 1], [boundLow[k], boundLow[k]],
                                      '--',
                                      color=BOUNDS_COLOR,
                                      alpha=LINE_ALPHA)
                    else:
                ...
                for s in range(quantity.shape[1]):
                    p, = ax[i, j].plot(quantity[:, s, k], line_styles[s], alpha=LINE_ALPHA, linewidth=lw)
                    if (margins is not None):
                        if (type(margins) is list):
                            mp = margins[0]
                            mn = margins[1]
                ...
                ax[i, j].xaxis.set_ticks(np.arange(0, quantity.shape[0], quantity.shape[0] / 2))
    ...
    if (SHOW_LEGENDS and solver_names is not None):
        #        leg = ax[0,0].legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=2, mode="expand",
        #        borderaxespad=0.)
...
def plotQuantityPerSolver(quantity, title, solver_names, line_styles, yscale='linear', ylabel=''):
    ...
    for i in range(len(solver_names)):
        ax.plot(quantity[:, i], line_styles[i], alpha=LINE_ALPHA, linewidth=lw)
    ...
    if (SHOW_LEGENDS):
        leg = ax.legend(solver_names, loc='best')
        leg.get_frame().set_alpha(LEGEND_ALPHA)
    if (SAVE_FIGURES):
        for ext in FILE_EXTENSIONS:
            plt.gcf().savefig(FIGURE_PATH + title.replace(' ', '_') + '.' + ext,
...
def plotQuantityVsQuantityPerSolver(quantity, quantityPerSolver, legend, solver_names, line_styles, yscale='linear'):
    ...
    if (SAVE_FIGURES):
        for ext in FILE_EXTENSIONS:
            f.savefig(FIGURE_PATH + (legend[0] + '_VS_' + legend[1]).replace(' ', '_') + '.' + ext,
                      format=ext,
                      dpi=FIGURES_DPI,
                      bbox_inches='tight')
    if (SHOW_LEGENDS):
