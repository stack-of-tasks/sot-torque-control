...
DQ_MAX = np.array([
    3.5, 3.8, 4.4, 3.9, 5.0, 9.1, 3.5, 3.8, 4.4, 3.9, 5.0, 9.1, 4.4, 2.3, 6.8, 6.8, 4.1, 2.4, 4.1, 2.2, 4.7, 3.1, 5.6,
...
if (USE_ROBOT_VIEWER):
    ...
    for i in range(T):
        ...
        if (i % VIEWER_PERIOD == 0):
            viewer.updateElementConfig('hrp_device', [0, 0, 0.7, 0, 0, 0] + q[i, :].tolist() + 10 * [
                0,
            ])
...
for j in range(NJ):
    print("Max velocity of joint %d as a percentage of its velocity limit: %f" %
          (j, 100 * np.max(dq[:, j]) / DQ_MAX[j]))
...
if (USE_ROBOT_VIEWER):
    ...
    for i in range(T_int):
        sleep(DT_DES)
        if (i % VIEWER_PERIOD == 0):
            viewer.updateElementConfig('hrp_device', [0, 0, 0.7, 0, 0, 0] + q_int[i, :].tolist() + 10 * [
                0,
            ])
