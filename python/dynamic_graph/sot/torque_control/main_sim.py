...
from create_entities_utils import (create_admittance_ctrl, create_ctrl_manager, create_estimators,
                                   create_inverse_dynamics, create_position_controller, create_torque_controller,
                                   create_tracer, create_trajectory_generator)
from dynamic_graph import plug
from dynamic_graph.sot.torque_control.hrp2_device_pos_ctrl import HRP2DevicePosCtrl
from hrp2_motors_parameters_sim import NJ, k_p, k_tau, k_v
    ...
    for i in range(1, N):
        ...
        if (i % estimationDelay == 0):
            ...
            if i > estimationDelay:
                print(
                    "\nTime %.3f) q est VS real = (%.2f, %.2f), dq est VS real (%.2f, %.2f) ddq est VS real (%.2f, %.2f)"  # noqa
                    % (i * dt, rad2deg * estimator_kin.x_filtered.value[j], q_real,
                       rad2deg * estimator_kin.dx.value[j], dq_real, rad2deg * estimator_kin.ddx.value[j], ddq_real))
...
def test_chirp(device, traj_gen, estimator_ft, dt=0.001):
    ...
    for i in range(N + 2):
        ...
        # (device.robotState.time * dt, device.robotState.value[6 + jid], traj_gen.q.value[jid]))
        # print("dq(%.3f) = %.3f,  \tdqDes  = %.3f" %
        # (device.robotState.time * dt, device.jointsVelocities.value[6 + jid], traj_gen.dq.value[jid]))
...
def test_sinusoid(device, traj_gen, dt=0.001):
    ...
    for i in range(4 * N):
        ...
        dq[i] = device.jointsVelocities.value[jid]
        if USE_ROBOT_VIEWER and i % 30 == 0:
            viewer.updateElementConfig('hrp_device', list(device.state.value) + [0.0] * 10)  # noqa
        if i % 100 == 0:
            print("q(%.3f) = %.3f, \tqDes = %.3f" %
                  (device.robotState.time * dt, device.robotState.value[6 + jid], device.control.value[jid]))
    traj_gen.stop('rhp')
...
def test_admittance_ctrl(device, ctrl_manager, traj_gen, estimator_ft, adm_ctrl, dt=0.001):
    ...
    traj_gen.startForceSinusoid('lf', axis, 100, 1.5)
...
def main(task='', dt=0.001, delay=0.01):
    ...
    inv_dyn.Kp.value = NJ * (1.0, )
    # 10
    inv_dyn.Kd.value = NJ * (2.0, )
    # 5
    ...
    # tracer.stop();
    # tracer.dump();
    return (device, estimator_ft, filters.estimator_kin, torque_ctrl, traj_gen, ctrl_manager, inv_dyn, pos_ctrl,
            adm_ctrl, tracer)


if __name__ == '__main__':
    (device, estimator_ft, estimator_kin, torque_ctrl, traj_gen, ctrl_manager, inv_dyn, pos_ctrl, adm_ctrl,
     tracer) = main('none', 0.001, 0.01)
    pass
