...
print('double range_to_limits_correction[30] =  {%s, // RLEG' % ', '.join(map(str, offsets_correction[0:6])))
print('                                          %s, // LLEG' % ', '.join(map(str, offsets_correction[6:12])))
print('                                          %s, // CHEST' % ', '.join(map(str, offsets_correction[12:14])))
print('                                          %s, // HEAD' % ', '.join(map(str, offsets_correction[14:16])))
# Mind the hack (-0.1) to prevent the hand to close too much in q0
print('                                          %s-0.1, // RARM' % ', '.join(map(str, offsets_correction[16:23])))
# Mind the hack (-0.1) to prevent the hand to close too much in q0
print('                                          %s-0.1};// LARM' % ', '.join(map(str, offsets_correction[23:30])))
